...
function reducePropsToArrayWithInitValue(userProps) {
  return Object.entries(userProps != null ? userProps : {}).reduce((acc, _ref) => {
    let [key, value] = _ref;
    acc[key] = [value];
    return acc;
  }, {});
  ...
  propsList.forEach(props => {
    ...
    Object.keys(elementProps).forEach(propKey => {
      if (typeof elementProps[propKey] === 'function') {
        var _mergePropsMap$elemen;

        if (mergePropsMap[elementKey][propKey] == null) {
          mergePropsMap[elementKey][propKey] = [];
        }

        (_mergePropsMap$elemen = mergePropsMap[elementKey][propKey]) == null ? void 0 : _mergePropsMap$elemen.push(elementProps[propKey]);
      }
    });
  });
...
function safePolygon(_temp) {
  ...
  return _ref => {
    ...
    return function onPointerMove(event) {
      ...
      const BUFFER = 1; // Within the rectangular trough between the two elements
...
const useHover = function (context, _temp) {
  ...
  }, [open, enabled, refs.floating]); // Registering the mouse events on the reference directly to bypass React's
...
const useFocusTrap = function (_ref, _temp) {
  ...
  function onBlur(event) {
    ...
    if (!((_refs$floating$curren3 = refs.floating.current) != null && _refs$floating$curren3.contains(target)) && isElement(refs.reference.current) && !refs.reference.current.contains(target)) {
      onOpenChange(false);
    }
  }
